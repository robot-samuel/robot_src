...
{
	ttyport ="can0";
 	onlineMsg.online=FN_LOST;
	Fd133Driver.Set_add((unsigned char)1);
	Fd133Driver.set_dir(1);
	add = 101;
	velocity =0;
	
	
}
kincocanfd133Mode::kincocanfd133Mode(std::string port,int devadd,int dir)
:ttyport(port)
{
...
void kincocanfd133Mode::Net2Ctrl_driverCall(const dgvmsg::DriverVelocity::ConstPtr& cmdmsg)
{
	...
	for(int index=0;index<size;index++){
			...
			if(cmdmsg->driver[index].add == add){
	...
	DriveInit(vecpa);
        ...
        ros::Rate loop_rate(10);
         ...
         try{
			...
			//驱动器报错
		 	if(Fd133Driver.m_BitStatus.bit3_isFault ==1 ){
					statues_ptr=FUN_REENABLE;
		                }                       
                        //驱动器状态失联
...
void kincocanfd133Mode::SetSpeed(int speed)
{
...
void kincocanfd133Mode::StatusRecord( boost::shared_ptr<socketcan> &vecpa){
...
void kincocanfd133Mode::DevGetUdc(std::vector < boost::shared_ptr<socketcan> > &vecpa){
	...
	vecpa[0]->clear();
	vecpa[0]->sendRequest(Fd133Driver.Cmd_GetUdc());
	boost::this_thread::sleep(boost::posix_time::milliseconds(1));
...
void kincocanfd133Mode::DevGetCurCurent(std::vector < boost::shared_ptr<socketcan> > &vecpa)
{
	...
	vecpa[0]->clear();
	vecpa[0]->sendRequest(Fd133Driver.Cmd_GetCurrent());
	boost::this_thread::sleep(boost::posix_time::milliseconds(1));	
...
void kincocanfd133Mode::DevGetActPos(std::vector < boost::shared_ptr<socketcan> > &vecpa)
{
	...
	vecpa[0]->clear();
	vecpa[0]->sendRequest(Fd133Driver.Cmd_GetCruPos());
	boost::this_thread::sleep(boost::posix_time::milliseconds(1));	
...
void kincocanfd133Mode::DevGetVelocity(std::vector < boost::shared_ptr<socketcan> > &vecpa)
{
	...
	vecpa[0]->clear();
	vecpa[0]->sendRequest(Fd133Driver.Cmd_GetVelocity());
	boost::this_thread::sleep(boost::posix_time::milliseconds(1));	
...
void kincocanfd133Mode::GetCruError1(std::vector < boost::shared_ptr<socketcan> > &vecpa){
   ...
   //发送
	...
	vecpa[0]->clear();
	vecpa[0]->sendRequest(Fd133Driver.Cmd_GetError());
	boost::this_thread::sleep(boost::posix_time::milliseconds(1));	
...
void kincocanfd133Mode::GetSerialNo(std::vector < boost::shared_ptr<socketcan> > &vecpa)
{
   ...
   //发送
	...
	vecpa[0]->clear();
	vecpa[0]->sendRequest(Fd133Driver.Cmd_GetDevserialN());
	boost::this_thread::sleep(boost::posix_time::milliseconds(1));
...
void kincocanfd133Mode::DevSetspeed(std::vector < boost::shared_ptr<socketcan> > &vecpa,int vec_speed){
	...
	vecpa[0]->clear();
	vecpa[0]->sendRequest(Fd133Driver.Cmd_SetVelocity(vec_speed));
	boost::this_thread::sleep(boost::posix_time::milliseconds(1));	
...
void kincocanfd133Mode::DevCtrleEndable(std::vector < boost::shared_ptr<socketcan> > &vecpa){
	...
	vecpa[0]->clear();
	vecpa[0]->sendRequest(Fd133Driver.Cmd_SetEnable());
	boost::this_thread::sleep(boost::posix_time::milliseconds(1));	
...
void kincocanfd133Mode::DevCtrlDisEndable(std::vector < boost::shared_ptr<socketcan> > &vecpa){
	...
	vecpa[0]->clear();
	vecpa[0]->sendRequest(Fd133Driver.Cmd_SetRealse());
	boost::this_thread::sleep(boost::posix_time::milliseconds(1));	
...
void kincocanfd133Mode::DevSetClearError(std::vector < boost::shared_ptr<socketcan> > &vecpa){
	...
	vecpa[0]->clear();
	vecpa[0]->sendRequest(Fd133Driver.Cmd_SetClearError());
	boost::this_thread::sleep(boost::posix_time::milliseconds(1));	
...
void kincocanfd133Mode::CmdStop(std::vector < boost::shared_ptr<socketcan> > &vecpa){
	...
	vecpa[0]->clear();
	vecpa[0]->sendRequest(Fd133Driver.Cmd_SetStop());
	boost::this_thread::sleep(boost::posix_time::milliseconds(1));	
...
void kincocanfd133Mode::DevEmergencyStop(std::vector < boost::shared_ptr<socketcan> > &vecpa){
	...
	vecpa[0]->clear();
	vecpa[0]->sendRequest(Fd133Driver.Cmd_QuickStop());
	boost::this_thread::sleep(boost::posix_time::milliseconds(1));		
...
void kincocanfd133Mode::DevGetDrivStatus(std::vector < boost::shared_ptr<socketcan> > &vecpa){
	...
	vecpa[0]->clear();
	vecpa[0]->sendRequest(Fd133Driver.Cmd_GetStauts());
	boost::this_thread::sleep(boost::posix_time::milliseconds(1));
...
void kincocanfd133Mode::GetSpeedCmd(std::vector < boost::shared_ptr<socketcan> > &vecpa){
	...
	vecpa[0]->clear();
	vecpa[0]->sendRequest(Fd133Driver.Cmd_GetCmdVelocity());
	boost::this_thread::sleep(boost::posix_time::milliseconds(1));
...
void kincocanfd133Mode::dirv_pub_encounter()
{   
	dgvmsg::Encounter encounter;
	
	//encounter.devadd = driverprco.add();
	encounter.devadd = add;
	encounter.encounter = Fd133Driver.CruPos();
...
void kincocanfd133Mode::dirv_pub_allstatues()
{   
...
std::string  kincocanfd133Mode::Pack_driver_statues()
{       
         // Verify that the version of the library that we linked against is
      // compatible with the version of the headers we compiled against.
      GOOGLE_PROTOBUF_VERIFY_VERSION;
      ...
      driver_status.set_device_add((::google::protobuf::int32)add);
       driver_status.set_actvelocity(Fd133Driver.cru_speed()*Fd133Driver.dir());
       driver_status.set_setvelocity(Fd133Driver.cru_speedcmd()*Fd133Driver.dir());
       driver_status.set_ctrl_dc_voltage(Fd133Driver.Udc());

       driver_status.set_ctrl_ipm_temperature(Fd133Driver.IpmTem());
       driver_status.set_actcurrent((float)Fd133Driver.effectiveTorque_I());
       
       driver_status.set_actualpos(Fd133Driver.CruPos()*Fd133Driver.dir());
       //driver_status.set_emergencyinputvalue(driverprco.emergencyinputvalue());
      //driver_status.clear_motec();
      
       driver_status.set_error(Fd133Driver.GetStrError());
