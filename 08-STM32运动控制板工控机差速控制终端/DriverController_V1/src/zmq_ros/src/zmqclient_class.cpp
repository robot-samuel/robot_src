	...
	//zmq::pollitem_t items[] = {{this->zmqsub_, 0, ZMQ_POLLIN, 0}};
        ...
        while (ros::ok()) 
        {
	      ...
	      if(connectflag == true){  
			     ...
			     this->zmqsub_->recv(&topicmessage,ZMQ_DONTWAIT);
			     this->zmqsub_->getsockopt( ZMQ_RCVMORE, &more, &more_size);
		            if (more){
				     int setpmore=0;
	                    	     std::size_t setpmore_size = sizeof (setpmore);
		                    zmq::message_t setpmessage;
				     this->zmqsub_->recv(&setpmessage,ZMQ_DONTWAIT);
				     this->zmqsub_->getsockopt( ZMQ_RCVMORE, &setpmore, &setpmore_size);
				     if (setpmore){ 
					     int contextmore=0;
		                    	     std::size_t contextmore_size = sizeof (contextmore);
			                    zmq::message_t contextmessage;
					     this->zmqsub_->recv(&contextmessage,ZMQ_DONTWAIT);
					     this->zmqsub_->getsockopt( ZMQ_RCVMORE, &contextmore, &contextmore_size);	
					     if(!contextmore){
							//接收处理信息
								std::cout << "i recv a heart"<< std::endl;
								std::string context = std::string(static_cast<char*>(contextmessage.data()), contextmessage.size());
